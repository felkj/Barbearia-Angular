...
import { IDialogManegerService } from '../../../services/idialog-maneger.service';
...
export class ClientTableComponent implements AfterViewInit, OnChanges, OnDestroy{
  ...
  private dialogManegerServiceSubcription?: Subscription
  ...
  constructor(
    @Inject(SERVICES_TOKEN.DIALOG) private readonly dialogManegerService: IDialogManegerService
  ){}
  ...
  ngOnDestroy(): void {
    if(this.dialogManegerServiceSubcription){
      this.dialogManegerServiceSubcription.unsubscribe()
    }
  }
  ...
  deleteClient(client: ClientModelTable){
    this.dialogManegerService.showYesNoDialog(
